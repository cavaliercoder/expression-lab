...
namespace DigitalRune.Windows.TextEditor
{
  ...
  internal class LineNumberMargin : AbstractMargin
  {
    ...
    {
        using (Stream cursorStream = Assembly.GetCallingAssembly().GetManifestResourceStream("DigitalRune.Windows.TextEditor.Resources.RightArrow.cur"))
        {
            if (cursorStream != null)
                _rightLeftCursor = new Cursor(cursorStream);
            else
                _rightLeftCursor = Cursors.Default;
        }
    }
